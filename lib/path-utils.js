...
Poly.prototype.contains = function(x, y) {
...
Poly.prototype.drawPoints = function(pointSz) {
  ...
  var s = pointSz || 2;
  for (var j = 0; j < this.points.length; j++) {
    ellipse(this.points[j].x, this.points[j].y, s, s);
  }
  if (this.holes) {
    for (var i = 0; i < this.holes.length; i++) {
      for (var k = 0; k < this.holes[i].length; k++)
        ellipse(this.holes[i][k].x, this.holes[i][k].y, s, s);
    }
  }
}
 ...
 * @param  {boolean} simplify    attempt to simplify the path if true
 * @param  {number} numSamples  the max number of points to be returned
 * @return {array}  an array of hierarchical Poly objects
 */
function getPolys(glyph, simplify, numSamples) {
  ...
  for (var j = 0; j < paths.length; j++) {
    var pts = pointsFromPath(paths[j], simplify, numSamples);
    polys.push(new Poly(pts));
  }

  for (var j = 0; j < polys.length; j++) {
    for (var i = j-1; i >= 0; i--) {
      if (polyInPoly(polys[i].points, polys[j].points)) {
 ...
 * @param  {[type]} pts  [polygon pts]
 * @param  {[type]} x  [point-x]
 * @param  {[type]} y  [point-y]
 * @param  {[type]} bbox [optional, for optimization]
...
/*
 * Naive check if polygon2 is inside polygon1 (should also check intersection)
 ...
 */
function polyInPoly(pts1 /* polygon1 */ , pts2 /* polygon2 */ ) {

  var bb1 = pathBounds(pts1),
  bb2 = pathBounds(pts2),
  pIn = pointInBox;
  ...
  if (!pointInPoly(pts2[i].x, pts2[i].y, pts1))
    return false;
  }
...
function getLengthFactory(istotal, subpath) {
  ...
  subpaths.end = sp;
  point = istotal ? len : subpath ? subpaths : R.findDotsAtSegment(x, y,
...
function catmullRom2bezier(crp, z) {
  ...
  d.push(["C", (-p[0].x + 6 * p[1].x + p[2].x) / 6,
    (-p[0].y + 6 * p[1].y + p[2].y) / 6, (p[1].x + 6 * p[2].x - p[3].x) / 6,
