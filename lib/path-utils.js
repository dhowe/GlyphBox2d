...
Poly.prototype.getPoints = function(options) {
  ...
  if (0&&this.holes && opts.includeHoles) {
 ...
 */
function getPolys(glyph, options) {
...
function parsePathPoints(cmds) {

  console.log('parsePathPoints: '+cmds.length);
  var pathPts = [];
  ...
  for (var i = 0; i < cmds.length; i++) {
    ...
    console.log(cmds[i]);
    if (t === 'M' || t === 'L' || t === 'C' || t === 'Q') {
      console.log(cmds[i].x1, cmds[i].y1);
      pathPts.push({ x: cmds[i].x1, y: cmds[i].y1 });
      //noFill();
      //stroke(0,255,0);
      ellipse(cmds[i].x1, cmds[i].y1, 1, 1);
