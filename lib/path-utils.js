...
Poly.prototype.triangulateWithPoly2Tri = function(angle) { // requires poly2tri.js

  // var cloned = [];
  // for (var i = 0; i < this.points.length; i++) {
  //   cloned.push({x:this.points[i].x,y:this.points[i].y});
  // }
  ...
  //console.log('Poly2tri found '+tris.length+
    //' tris from '+this.points.length + ' points');
...
Poly.prototype.triangulate = function(angle, noHoles) { // requires earcut
  ...
  console.log('found '+this.triangles.length+
    ' tris from '+this.points.length + ' points');

  return this.triangles;
}

Poly.prototype.triangulateWithTessLib = function(angle) { // requires tesslib
...
Poly.prototype.getPoints = function(options) {
  ...
  if (0&&this.holes && opts.includeHoles) { // TODO: test/fix
 ...
 * more precise
 * <br>removeCollinears - if set to a non-zero value, collinear points will be
...
function getPolys(glyph, x, y, fontSize, options) {
  ...
  //console.log('getPolys: ',x,y,fontSize,options);

  var gpath = glyph.getPath(x, y, fontSize),
    //pathPts = parsePathPoints(gpath.commands),
    paths = splitPaths(gpath.toPathData()),
    polys = [];

  for (var j = 0; j < paths.length; j++) {
    var pts = pointsFromPath(paths[j], options);
...
function pointsFromPath(pathDataStr, options) {
  ...
  if (opts.removeCollinears) {

    var count = removeCollinears(pts, opts.removeCollinears);
 ...
 * @param  {Number} angle - The threshold angle to use when determining
 *   whether two edges are collinear. Use zero for finest precision.
 * @return {Number}       The number of points removed
 * TODO: never remove points on original curve!
 */
function removeCollinears(pts, angle, originalCurvePts) {
