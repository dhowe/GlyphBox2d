 ...
 */
function pointInPoly(pts, x, y, bbox) {

  // ray-casting based on
  // http://www.ecse.rpi.edu/Homepages/wrf/Research/Short_Notes/pnpoly.html

  var inside = false;
  if (!bbox || pointInBox(bbox, x, y)) {
...
function polyInPoly(pts1 /* polygon1 */ , pts2 /* polygon2 */ ) {
  ...
  for (var i = 0; i < pts2.length; i++) {
    if (!pointInPoly(pts1, pts2[i].x, pts2[i].y))
      return false;
  }
...
function pointInPolyX(path, x, y, bbox) {

  bbox = bbox || pathBounds(path);
  var inbox = pointInBox(bbox, x, y)
  if (inbox) {
    return pathHelper(path, [
      ["M", x, y],
      ["H", bbox.x2 + 10]
    ], true) % 2 == 1;
  }
...
function pointInBox(bbox, x, y) { // takes x,y,x2,y2 (CORNERS)
...
function pathHelper(bez1, bez2, justCount) {
