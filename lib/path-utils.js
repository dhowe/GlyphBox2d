...
function Poly(pts) {
  ...
  this.triangles = undefined; // 1d array of decomposed triangle objects
}
...
Poly.prototype.triangulate = function(noHoles) { // requires earcut

  return this.triangulatePoly2Tri(noHoles);
}

Poly.prototype.triangulatePoly2Tri = function(noHoles) { // requires poly2tri.js

  console.log('triangulatePoly2Tri');
...
Poly.prototype.triangulatePnltri = function(noHoles) {

  console.log('triangulatePnltri');
...
Poly.prototype.triangulateDelaunay = function(noHoles) {

  console.log('triangulateDelaunay');
  var contour = [],  holeIndices;

  for (var j = 0; j < this.points.length; j++) {
    contour.push([this.points[j].x, this.points[j].y]);
  }
  ...
  if (this.holes && !noHoles) {
    ...
    for (var i = 0; i < this.holes.length; i++) {
      ...
      for (var j = 0; j < hpts.length; j++) {
        contour.push([hpts[j].x, hpts[j].y]);
      }
    }
  }

  var triIdxs = Delaunay.triangulate(contour);//, holeIndices);

  this.triangles = [];
  for (var i = 0; i < triIdxs.length; i+=3) {

    var pts = [
      { x: contour[ triIdxs[i]   ][0], y:  contour[ triIdxs[i]   ][1] },
      { x: contour[ triIdxs[i+1] ][0], y:  contour[ triIdxs[i+1] ][1] },
      { x: contour[ triIdxs[i+2] ][0], y:  contour[ triIdxs[i+2] ][1] }
    ];
...
Poly.prototype.triangulateEarcut = function(noHoles) { // requires earcut
  console.log('triangulateEarcut');
  var contour = [],  holeIndices;
  ...
  var triIdxs = earcut(contour, holeIndices);
...
Poly.prototype.decompose = function(noHoles) {

  console.log('triangulateDecomp');
  this.points.reverse();
  this.triangles = [];
  var concave = new decomp.Polygon();
  for (var k = 0; k < this.points.length; k++) {
    concave.vertices.push([this.points[k].x, this.points[k].y]);
  }
  ...
  for (var i = 0; i < plys.length; i++) {
    ...
    for (var j = 0; j < verts.length; j++) {
      var pt = { x: verts[j][0], y: verts[j][1] };
      //console.log(pt);
      pts.push(pt);
    }
    this.triangles.push(pts);
...
Poly.prototype.triangulateTess = function(noHoles) { // requires tesslib

  console.log('triangulateTess');
  ...
  //console.log(contour);
  var tarr = triangulate(contour);
  //console.log(tarr);
  this.triangles = [];
  for (var i = 0; i < tarr.length; i+=3) {

    var pts = [ tarr[i], tarr[i+1], tarr[i+2] ];
    this.triangles.push(pts);
  }
  return this.triangles;
}
...
Poly.prototype.contains = function() {
  ...
  if (arguments.length > 1 && typeof arguments[0].x != 'undefined')
    return this._containsPoint(arguments[0], arguments[1]);

  return polyInPoly(this, arguments[0]);
}

Poly.prototype._containsPoint = function(x, y) {
...
Poly.prototype.getPoints = function(options) {
  ...
  var pts = [], opts = parseOpts(options, { includeHoles: true });
  for (var j = 0; j < this.points.length; j++) {
...
Poly.prototype.drawPoints = function(pointSz) {
 ...
 */
function getPolys(glyph, x, y, fontSize, options) {
  ...
  //console.log('getPolys: ',glyph, x, y, fontSize, opts);
  ...
  for (var j = 0; j < paths.length; j++) {
    var pts = pointsFromPath(paths[j], opts);
...
/**
 * Converts a path-string to a set of (polygon) points
 * @param  {string} pathDataStr an svg-style path-data string
 * @param {object} Options is an optional object that contains:
 * <br>sampleFactor - the ratio of path-length to number of samples
 * (default=.25); higher values yield more points and are therefore
 * more precise
 * <br>simplifyThreshold - if set to a non-zero value, collinear points will be
 ...
 */
function pointsFromPath(pathDataStr, options) {
  ...
  for (var i = 0; i < len; i += t)
    pts.push(getPointAtLength(cmds, i));
...
function polyInPoly(p1, p2) {
  ...
  //console.log('polyInPoly', typeof p1, typeof p2);
        ...
        if (len + l > length) {
          if (subpath && !subpaths.start) {
            point = getPointAtSegmentLength(x, y, p[1], p[2], p[3], p[4], p[5],
              p[6], length - len);
            sp += ["C" + point.start.x, point.start.y, point.m.x, point.m.y,
              point.x, point.y
            ];
            if (onlystart) {
              return sp;
            ...
            subpaths.start = sp;
            sp = ["M" + point.x, point.y + "C" + point.n.x, point.n.y,
              point.end.x, point.end.y, p[5], p[6]
            ].join();
            len += l;
            x = +p[5];
            y = +p[6];
...
function parsePathString(pathString) { // TODO: remove
